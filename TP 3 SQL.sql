...
SELECT users.username, comments.contents FROM users JOIN comments ON users.id=comments.user_id;

-- Pour chaque commentaire, afficher son contenu et l'url de la photo à laquel le commentaire a été ajouté
SELECT comments.contents, photos.url FROM photos JOIN comments ON photos.id=comments.photo_id;

-- Afficher l'url de chaque photo et le nom d'utilisateur de l'auteur
SELECT photos.url, users.username FROM users JOIN photos ON users.id=photos.user_id;

-- Trouver tous les commentaires pour la photo d'id 3, avec le username de l'utilisateur qui a commenté
SELECT comments.contents, users.username FROM comments JOIN users ON comments.user_id=users.id JOIN photos ON comments.photo_id=photos.id WHERE photos.id=3;
...
SELECT photos.url, comments.contents, users.username FROM photos JOIN users ON photos.user_id=users.id JOIN comments ON photos.id=comments.photo_id WHERE users.id=photos.user_id;
